...
int main(int argc, char *argv[])
{
  ...
  static const std::string results_folder_name = "./Results/Sink";
  ...
  // log.writeandshout("Input: " + std::string(argv[1])+"\n" );
  // const double modif = ((double)std::stol(argv[1], nullptr, 0))/((double)100);
  ...
  double p0[3] = {0.3,0.3,0. }; eps_vpointfile.write("("+to_string(p0[0]) + ";"+to_string(p0[1]) + ";"+ to_string(p0[2]) + ")" );
  ...
  double p1[3] = {2.,0.3,0. }; eps_vpointfile.write("("+to_string(p1[0]) + ";"+to_string(p1[1]) + ";"+ to_string(p1[2]) + ")" );
  ...
  double p2[3] = {2.,3.7,0. }; eps_vpointfile.write("("+to_string(p2[0]) + ";"+to_string(p2[1]) + ";"+ to_string(p2[2]) + ")" );
  ...
  double p3[3] = {0.3,4.,0. }; eps_vpointfile.write("("+to_string(p3[0]) + ";"+to_string(p3[1]) + ";"+ to_string(p3[2]) + ")" );
  ...
  // Pressure increment
  double p_incr = (-0.01)/(1./1.); //modif = 1./1.
  int n_incr = (200*1) /1; //modif = 1./1.
  ...
  *surfMeasGaz = 0;
  ...
  auto PlNullBot = std::make_shared<DirichletBC>(W->sub(1), p_null, botCenter);
  auto PgNullBot = std::make_shared<DirichletBC>(W->sub(2), p_null, bot);
  auto PlRampBot = std::make_shared<DirichletBC>(W->sub(1), p_ramp, botCenter);
  ...
  while (t <= duration)
  {
    ...
    *wDualt = Function(*wDual);
    do{
      ...
      *PlNullBot = DirichletBC(W->sub(1), p_null, botCenter);
      ...
      *PlRampBot = DirichletBC(W->sub(1), p_ramp, botCenter);
